...
2. **Admin replies** in natural language (e.g. "Tuesday and Saturday", "No games this week") — `admin-processor` calls Bedrock to parse `SCHEDULE_GAMES` (one or more `{date, time}`) or `NO_GAMES_THIS_WEEK`. For each scheduled game it creates the DynamoDB record and starts a Step Functions execution named `game-{gameDate}`.
...
4. **Per-game Step Functions execution** (`basketball-game-lifecycle`) drives the lifecycle via 4 Lambda tasks, each gated by the game still being `OPEN`:
   - `announce_task` (T-7d) — tentative announcement with duration hint
   - `reminder_task` (T-4d) — low-signup reminder if confirmed < `MIN_PLAYERS`
   - `confirm_or_cancel_task` (T-2d) — go/no-go decision; locks duration (1hr if confirmed < `LONG_GAME_THRESHOLD`, else 2hr)
...
- `bedrock_client.py` — Bedrock integration; `parse_player_email` returns structured JSON `{intent, guests, confirmed_guest_names, query_target, reply_draft}`; `parse_admin_email` returns `{intent, game_date, email, name, is_admin, games}`
...
Key variables (set in `terraform.tfvars`): `domain_name`, `sender_email`, `admin_email`, `game_time`, `game_location`, `bedrock_model_id`, `min_players`, `long_game_threshold`, `max_games_per_week`.
